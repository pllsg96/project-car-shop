...
class MotorcycleController {
  ...
  public async updateMotoById(req: Request, res: Response, next: NextFunction) {
    const { id } = req.params;
    const { body } = req;
    try {
      const { message, status, result } = await this.service.updateMotoById(id, body);
      if (message) return res.status(status).json({ message });
      return res.status(status).json(result);
    } catch (error) {
      return next(error);
  ...
  public async deleteMotoById(req: Request, res: Response, next: NextFunction) {
    const { id } = req.params;
    try {
      const { message, status } = await this.service.deleteMotoById(id);
      if (message) return res.status(status).json({ message });
